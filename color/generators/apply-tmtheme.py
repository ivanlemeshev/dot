...
try:
    ...
    sys.exit(1)
spec_file = os.path.join(os.path.dirname(__file__), "..", "templates", "tmtheme_spec.json")
...
for item in spec["global_settings"]:
    ...
    global_settings[item["key"]] = colors[item["color"]] + suffix

settings = [{"settings": global_settings}]
for entry in spec["entries"]:
    entry_settings = {"foreground": colors[entry["color"]]}
