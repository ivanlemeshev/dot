...
The Fedora KDE Plasma developer desktop profile includes Ghostty, Neovim, Fish, Git, GitHub CLI, `bat`, `fd`, `fzf`, `jq`, `ripgrep`, `mise`, Tmux, LazyGit, Lazydocker, `yt-dlp`, and Cliamp. It also includes rootless Docker with Docker Compose. It uses a plain Fish prompt and the shared font stack. On Windows, Fish runs in MSYS2 while PowerShell remains available for Windows-only work.
