...
Produce a concise summary that keeps the document's material meaning. Do not treat a short summary as a list of headings.
...
Make a coverage ledger before you write the final summary. Record the material point from each section, including:
...
Read long documents in logical sections. Summarize each section before you merge them. Keep the ledger until the final coverage check is complete.

## Write the summary

Start with the document title, type, and main purpose. Then state the main conclusion or message. Group the remaining points by the source document's major topics.
...
Use headings when they make a long summary easier to scan. Cite source pages for material claims when the user asks for citations, when the document is technical or regulated, or when page tracing improves usefulness.

Keep the summary as short as the requested length permits. If the user asks for both extreme brevity and full detail, retain every material point and say that complete coverage requires a longer summary.

## Check before delivery

Compare the final summary with the coverage ledger. Confirm that every material point appears, is combined with an equivalent point, or is deliberately omitted because it is duplicate or non-material.

Check that no warning, exception, requirement, numerical limit, or conclusion is lost. Check all quoted facts against the PDF. Make the final text clear to its intended reader.
