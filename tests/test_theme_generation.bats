...
@test "apply-color-scheme reproduces checked-in generated theme files" {
  run bash "$TEST_ROOT/bin/apply-color-scheme" "$TEST_ROOT/color/schemes/color-scheme.kanagawa-dragon-contrast.yaml"
  [ "$status" -eq 0 ]
...
@test "template-driven generators can render missing target files from scratch" {
  ...
  run python3 "$TEST_ROOT/color/generators/apply-omp-theme.py" \
    "$TEST_ROOT/color/schemes/color-scheme.kanagawa-dragon-contrast.yaml" \
    "$TEST_ROOT/.config/oh-my-posh/theme.omp.json"
  [ "$status" -eq 0 ]
